...
def regenerate_client_secret(token, client_id):
    """Regenerate the client secret for the given client ID."""
    url = f"{KEYCLOAK_URL}/admin/realms/{KEYCLOAK_REALM}/clients/{client_id}/client-secret"
    headers = {"Authorization": f"Bearer {token}"}
    r = requests.post(url, headers=headers)
    if r.status_code != 200:
        print(f"Failed to regenerate client secret (HTTP {r.status_code})")
        return None
    secret = r.json().get("value")
    if not secret:
        print("No client secret returned after regeneration.")
        return None
    print("Client secret regenerated.")
    return secret
...
if __name__ == "__main__":
    ...
    # Regenerate client secret before fetching it
    regenerate_client_secret(token, client_id)
